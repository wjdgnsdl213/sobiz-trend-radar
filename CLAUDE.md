...
4. ✅ **지역×음식 크로스 분석**: `src/cross_region.py`, gazetteer(지역 사전) 기반 공기 빈도(NER 모델 미사용)
...
- 엔진 B (블로그, 소비 유행/신조어): buzz_collect → buzz_extract → buzz_validate(DataLab) → cross_region(지역×음식)
