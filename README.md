...
누적 DB(`data/trends.db`)를 읽어 두 엔진 트렌드를 시각화한다.
...
- **신조어 판별**: kiwipiepy `.oov`(사전 미등재)로 신조어(왁뿌·두쫀쿠)·합성어(소금빵)·일반어(디저트)를 구분한다. 다만 "두바이 초콜릿"처럼 기존 단어가 새 유행 의미로 쓰이는 경우는 단어 수준에서 잡지 못한다(일반어로 분류됨).
- **지역×음식 크로스 한계**: 지역 사전은 gazetteer라 미등록 신생 상권은 config에 추가해야 잡힌다. 부분문자열 매칭이라 동음 지명 오탐 여지도 있어, 지역 내 비중(distinctiveness)을 함께 봐야 한다. soynlp 파편은 buzz_extract 단계에서 대부분 제거된다.
...
PoC 검증(엔진 A precision 92.1%, 엔진 B 신조어 10개+ 발굴)을 마치고 운영 도구화 단계다.
